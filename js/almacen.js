...
const CAMPOS_FORM = [
    'txtNombre', 'cmbCategoria', 'cmbTallaSimple', 'btnAgregarTalla',
    'txtPrecio', 'txtColores', 'txtImagen', 'btnGenerarVariantes'
];
...
function renderTallasPorCategoria(categoria = '', tallasSeleccionadasParam = []) {
    const tallas = obtenerTallasPorCategoria(categoria);
    const tallasNormalizadas = [...new Set(tallasSeleccionadasParam.map(normalizarTalla))];
...
function agregarTallaSeleccionada() {
    ...
    if (tallasSeleccionadas.includes(tallaNueva)) return;
...
function renderProductos(productos = []) {
    ...
    if (contador) contador.textContent = `${productos.length} producto${productos.length !== 1 ? 's' : ''}`;
}
...
['filtroCat', 'filtroTalla', 'filtroStock'].forEach((id) => {
...
document.getElementById('btnLimpiarFiltros')?.addEventListener('click', () => {
    ...
    const b = document.getElementById('filtroBuscar');
    if (b) b.value = '';
    aplicarFiltros();
});

// ── 1. LEER Y MOSTRAR TABLA ──────────────────────────────────────
todosLosProductos = cargarProductosCache();
aplicarFiltros();
...
onSnapshot(
    ...
    (snapshot) => {
        ...
        guardarProductosCache(todosLosProductos);
        aplicarFiltros();
    },
    () => {
        todosLosProductos = cargarProductosCache();
...
btnGuardar.addEventListener('click', async () => {
    ...
    if (!nombre || !categoria) { alert("Rellena nombre y categoria"); return; }
    if (variantesValidas.length === 0) { alert("Genera al menos una variante con stock."); return; }
    ...
    if (!tallasDisponibles.every((t) => tallasValidas.includes(normalizarTalla(t)))) {
        alert("Hay tallas en variantes que no corresponden con la categoria elegida.");
        return;
    }
    if (precio === "" || isNaN(Number(precio)) || Number(precio) < 0) {
        alert("Introduce un precio válido (ej: 19.99)");
        return;
    }
...
document.getElementById('tablaProductos').addEventListener('click', async (e) => {
    ...
    try {
        ...
        let variantes = [];
        try { variantes = d.variantes ? JSON.parse(d.variantes) : []; } catch { variantes = []; }

        renderTallasPorCategoria(d.categoria, variantes.map((v) => v.talla));
        renderTablaVariantes(variantes);
